...
export function getBookingChangeImpact({ originalEvent, nextBooking }) {
  ...
  const newPrice = normalizeMoney(nextBooking.price);
  ...
  if (scheduleChanged) {
    changes.push("jadwal berubah");
  }

  if (roomChanged) {
    changes.push("room berubah");
  }

  if (packageChanged) {
    changes.push("paket berubah");
  }

  if (priceChanged) {
    changes.push(priceDelta > 0 ? "harga naik" : "harga turun");
  }

  if (paidChanged) {
    changes.push(paidDelta > 0 ? "pembayaran bertambah" : "pembayaran berkurang");
  }

  const suggestedPaymentStatus = getSuggestedPaymentStatus({
    price: newPrice,
    deposit: paidAmount,
  });
